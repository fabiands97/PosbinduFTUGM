<template>
  <div class="content-wrapper" style="min-height: 836px;">
    ...
    <section class="content-header">
      <h1>
        Kolesterol
        <small>Grafik Kolesterol Peserta Posbindu</small>
      </h1>
      <ol class="breadcrumb">
        <li>
...
export default {
  ...
  data: () => ({
    ...
    deskripsiLine: null,
    deskripsiBar: null,
    JudulBar: "Perbandingan Rerata Kolesterol",
    JudulLine: "Trend Rerata Kolesterol",
    lokasi: "Seluruh FT",
    someList: [
  ...
  async created() {
    ...
    try {
      const barData = await axios.get(
        "http://52.187.24.10:8080/api/v1/manajer/report/pengukuran/bar?type=Kolesterol"
      );
      const pieData = await axios.get(
        "http://52.187.24.10:8080/api/v1/manajer/report/pengukuran/pie?type=Kolesterol"
      );
      const lineData = await axios.get(
        "http://52.187.24.10:8080/api/v1/manajer/report/pengukuran/line?type=Kolesterol"
      );
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        this.deskripsiPie = pieData.data.deskripsi;
        this.deskripsiLine = lineData.data.deskripsi;
        this.deskripsiBar = lineData.data.deskripsi;
        this.loaded = true;
        console.log(this.lineData);
      } catch (e) {
        console.error(e);
