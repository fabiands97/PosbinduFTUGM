<template>
  <div class="content-wrapper" style="min-height: 836px;">
    ...
    <section class="content-header">
      ...
      <font color="red">Panduan Antrian pada Meja 3</font>
      <ul>
        <li>Ketikkan nama atau identitas peserta yang akan diukur</li>
        <li>Pilih tombol
        <font color="#00A65A"><b><i>'Input'</i></b></font>
          jika ingin melakukan input data</li>
        <li>Pilih tombol 
        <font color="#3C8DBC"><b><i>'Next'</i></b></font>
         jika peserta ingin tidak ingin melakukan pemeriksaan di Meja 3</li>
      </ul>
      </div>
    </section>
    ...
    >
      ...
      <template slot="actions" slot-scope="row">
        <router-link :to="{name: 'Input2', params: {user: row.item, nim : row.item['user.NoIdentitas']}}" class="btn btn-success"><i class="fa fa-edit">Input</i> </router-link>
...
export default {
 ...
 this.lokasi = this.$store.state.petugas.lokasi
    if(this.lokasi){
    axios
      .get("http://52.187.24.10:8080/api/v1/petugas/antrian/meja3/"+this.lokasi)
  ...
  methods: {
    ...
    nextMeja(userid){
            var user = {
             ...
             axios.post('http://52.187.24.10:8080/api/v1/petugas/antrian/meja3/next', user)
            .then(() => {
                this.$router.push('/list2')
                alert("Melanjutkan antrian peserta ke Meja 4")
                this.fetchAntrian()
